...
DENSE_CUDA_OBJ = denseam_kernel.cu.o gemm.cu.o approx_mul_lut.cu.o quant.cu.o transpose.cu.o
...
matmulam.o: matmulam.cc matmulam.h
	$(CXX) $(CFLAGS) $(CPPFLAGS)  $(ADD_ROUNDING_CPPFLAG)  $< -c -o $@
...
transpose.cu.o: cuda/transpose.cu cuda/error.cuh cuda/fp8_conversion.cuh
	$(NVCC) -x cu $(CUDA_CFLAGS) $(CPPFLAGS) -c $< -o $@
denseam_kernel.cu.o : cuda/denseam_kernel.cu cuda/error.cuh $(mul_inc_deps) cuda/fp8_conversion.cuh cuda/quant.cuh cuda/accumulate.cuh cuda/gemm_launcher.cuh cuda/transpose.cuh
